...
/*
	copy of RichEdit <- this has incredibly weird fucking logic
	
	I think that instead of trying to modify-to-taste it would be better to write one from scratch
...
class WPRuler : public FrameCtrl<Ctrl>
{
public:
...
public:
	struct Marker
	{
		...
		{
			return ((pos!=m.pos)||(minpos!=m.minpos)||(maxpos!=m.maxpos)||!image.IsSame(m.image)||(top!=m.top)||(deletable!=m.deletable));
		}
...
public:
	...
	void          SetLayout(int x, int pgcx, Zoom zoom, double grid, int numbers=INT_MAX, double numbermul=1, int marks=INT_MAX, int snap=32);
...
class WPUnitEdit : public EditField, public Convert
{
public:
...
public:
	WPUnitEdit& SetUnit(int _unit)                        { unit = _unit; return *this; }
	void      Set(int _unit, int d)                     { unit = _unit; SetData(d); }
	WPUnitEdit& WithSgn(bool b = true);

	typedef WPUnitEdit CLASSNAME;

	WPUnitEdit();
};

struct WPFontHeight : public WithDropChoice<EditDouble>
 {
	virtual bool Key(dword key, int);
	
	WPFontHeight()   { MinMax(1, 72); }
};

#define LAYOUTFILE <WordProcCtrl/wordprocctrl.lay>
#include <CtrlCore/lay.h>

bool WPEditHeaderFooter(String& header_qtf, String& footer_qtf);

class WPParaFormatting : public WithParaLayout<StaticRect>
{
public:
	DropList n[8];
...
private:
	...
	typedef WPParaFormatting CLASSNAME;
...
class WPStyleManager : public WithStylesLayout<TopWindow>
{
public:
...
private:
	...
	typedef WPStyleManager CLASSNAME;
...
public:
	...
	void     Setup(const Vector<int>& faces, int aunit = WP_UNIT_DOT);

	WPStyleManager();
};

//void WPSetupFaceList(DropList& face);

class WPEditor : public Ctrl, private TextArrayOps
{
public:
	virtual void  Layout();
	virtual void  Paint(Draw& w);
...
public:
	...
	void            Clear(bool bSetModified=true);
	...
	WPEditor&       NoRuler()                              { RemoveFrame(ruler); return *this; }
	WPEditor&       SingleLine(bool b = true)              { singleline = b; return *this; }
	WPEditor&       FontFaces(const Vector<int>& face);
	WPEditor&       ViewBorder(int cx)                     { viewborder = cx; Refresh(); return *this; }
	WPEditor&       ShowCodes(Color c)                     { showcodes = c; Refresh(); return *this; }
	WPEditor&       Unit(int u)                            { unit = u; Refresh(); return *this; }
	WPEditor&       SpellCheck(bool b)                     { spellcheck = b; Refresh(); return *this; }
	WPEditor&       SetZoom(int z)                         { zoom = z; Refresh(); return *this; }
	WPEditor&       SetContext(void *ctx)                  { context = ctx; Refresh(); return *this; }
	void           *GetContext() const                     { return context; }
	WPEditor&       ClipZoom(Zoom z)                       { clipzoom = z; return *this; }
	WPEditor&       ClipZoom(int m, int d)                 { clipzoom = Zoom(m, d); return *this; }
	Zoom            GetClipZoom() const                    { return clipzoom; }
	WPEditor&       BulletIndent(int i)                    { bullet_indent = i; return *this; }
	WPEditor&       PersistentFindReplace(bool b = true)   { persistent_findreplace = b; return *this; }
	WPEditor&       Floating(double zoomlevel_ = 1);
	WPEditor&       NoFloating(double zoomlevel_ = 1)      { return Floating(Null); }
	WPEditor&       SetPaintInfo(const PaintInfo pi)       { paint_info = pi; return *this; }
	WPEditor&       IgnorePhysicalObjectSize(bool b = true){ ignore_physical_size = b; return *this; }
	...
	typedef WPEditor CLASSNAME;

	WPEditor();
	virtual ~WPEditor();

	//for changing see ApplyStyleKey
//	void SetDefaultFace(const String sf, int h=10);
	
};

class WPEditCtrlWithToolBar : public WPEditor
{
	ToolBar  toolbar;
	bool     extended;
	void RefreshBar();
...
public:
	WPEditCtrlWithToolBar& Extended(bool b = true) { extended = b; return *this; }
	WPEditCtrlWithToolBar& NoExtended()            { return Extended(false); }

	typedef WPEditCtrlWithToolBar CLASSNAME;

	WPEditCtrlWithToolBar();
};

void WPAppendClipboard(RichText&& txt);
